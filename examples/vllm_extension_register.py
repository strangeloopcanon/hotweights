...
def install_extension_on_engine(
    engine, endpoint: str = "tcp://127.0.0.1:5555"
) -> HotweightsWorkerExtension:  # noqa: ANN001
    ext = HotweightsWorkerExtension(
        endpoint=endpoint, use_mpi=True, pinned=True, device="cuda"
    )
    # Bind module: try common attribute names
    mod = getattr(engine, "model", None) or getattr(
        getattr(engine, "model_runner", None), "model", None
    )
    if mod is None:
    ...
    engine.hotweights_extension = ext  # type: ignore[attr-defined]
    return ext


if __name__ == "__main__":
    print(
        "This module contains an example function; import and call "
        "install_extension_on_engine(engine)"
    )
